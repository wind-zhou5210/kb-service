...
>
> **v0.3.0（当前）**：`kb login` 默认走浏览器授权登录（授权码 + PKCE + 本机回环回调），新增 `--print-url`；
> 密码登录与环境变量方式保留。完整功能对齐（工作空间命令组、文档包等）见 v0.2.0。
...
- **v0.3.0** — `kb login` 默认改为浏览器授权（授权码 + PKCE + 本机回环回调），新增 `--print-url`；密码登录与环境变量方式保留
