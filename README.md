...
2. 触发服务器**主动拉取**源码（下载 GitHub 归档）
3. 服务器本地构建镜像并重启服务

> **为什么不让 CI 推送**：CI 运行在 GitHub 海外 runner，与国内服务器之间属跨境链路，
> 实测极不稳定——推镜像环节从 9.6 秒恶化到 30 分钟零进展，传源码环节从 44 秒恶化到
> 18 分钟未完成。改为「服务器主动拉取（跨境下行）+ 本地构建」后，跨境流量的方向与
> 体量都最小化，部署不再依赖不稳定的跨境上行链路。
...
| API | JWT 鉴权 + 集合归属校验；**读接口（集合列表、文档原文/下载/版本、工作空间文件）均需登录**，支持 Bearer / `?jwt=` / 会话 cookie 三种凭据 |
| CLI | 凭据存储在 `~/.kbconfig.json`，密码输入不回显；支持环境变量注入使凭据不落盘 |
| 分享 | 只读令牌（`secrets.token_urlsafe`），可一键撤销，撤销后原链接立即失效 |

**权限模型**：公开分享链接（`/api/share/*`、`/api/workspaces/share/*`）保持**刻意免登录**，接收方无需账号；其余读接口与全部写接口均需登录。
...
- [x] 工作空间安全整包更新（内容版本目录 + 原子指针切换，分享链接不变）
- [x] 读接口鉴权（会话 cookie 兼容 iframe 子资源，分享链接保持免登录）
- [x] CLI 浏览器授权登录（授权码 + PKCE + 本机回环回调）
- [ ] CI/CD 增加自动化测试 step
- [ ] 语雀文档批量导出迁移工具
- [ ] 登录系统升级（手机验证码登录）
- [ ] 引入向量知识库（ChromaDB）
- [ ] RAG 化改造（语义检索 + LLM 对话）
- [ ] CLI：目录同步 `kb sync`、Shell 补全、多 profile
