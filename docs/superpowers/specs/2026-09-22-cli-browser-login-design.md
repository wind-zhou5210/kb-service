...
3. 打开浏览器（地址取自配置的 KB_SERVER）───► GET {KB_SERVER}/login
                                          ?cli_callback=1
...
### 4.1.1 两个地址的角色（易混淆点，务必区分）

流程中出现两个不同性质的地址，容易被误认为“打开的是 localhost”：

| 地址 | 作用 | 谁在操作 | 用户感知 |
|------|------|----------|----------|
| `{KB_SERVER}/login?...` | **在浏览器中打开的登录页**——就是平时使用的那个服务登录页（地址取自 `KB_SERVER` 配置） | 用户手动输入账号密码 | 明确可见 |
| `http://127.0.0.1:<port>/callback` | **登录成功后浏览器自动跳转的回调地址**——CLI 在本机临时开启的接收端口，仅本机可达 | 浏览器自动跳转 | 几乎无感（页面一闪即完成） |

**为什么必须有本机回环这一步**：授权码需要从浏览器安全地“送回 CLI 进程”。CLI 运行在用户自己的电脑上（而非服务器），
服务器无法主动连接 CLI；因此只能由浏览器将授权码回传到本机回环地址（`127.0.0.1`），CLI 在那里接收。
这也是 OAuth 授权码流程的标准形态（`gh auth login` 同样回调到本机）。

> 结论：**打开的始终是你配置的服务器登录页**；localhost 仅是登录完成后的自动化回传环节，不需要你访问。
...
3. 打印授权链接（`{KB_SERVER}/login?...`，即配置的服务地址，而非本机地址）并尝试打开浏览器；
   打开命令按平台选择：macOS `open`、Windows `start`、Linux `xdg-open`
4. 等待回调（默认 5 分钟超时）：
